...
## 5. Armar los proyectos NetBeans

Resumen (pasos completos y por qué en `GUIA_EJECUCION.md`, sección "Concretando los ejercicios" / "## PC1"):

1. New Project → Java Application → nombre `MACRO-DATOS` (repetir para `PC1`).
...
3. Por cada carpeta en `source_Packages/`: crear un paquete con ese mismo nombre **sin el prefijo numérico** (ej. `01_TransaccionesPorPais` → paquete `TransaccionesPorPais`), y adentro 3 clases (`Mapper`, `Reducer`, `Driver`) con el código del repo pegado.
4. Build del proyecto → genera `dist\MACRO-DATOS.jar` / `dist\PC1.jar`.

## 6. Ejecutar los ejercicios

```powershell
.\hadoop.ps1 salesjam <NombrePaquete>                  # práctica, dataset SalesJan2009.csv
.\hadoop.ps1 pc1 <NombrePaquete> [argumentoOpcional]     # Laboratorio 01, dataset Turismo
```
